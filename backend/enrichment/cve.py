...
Design contract (mirrors the repo's preregistration discipline):
  * Additive: enrichment NEVER changes a finding's severity, weight, status,
    ...
    optional `cves` list to a finding DTO (purely informational). Enriching the
    same set of findings must always produce a byte-identical (score, verdicts,
    counts) result -- enrichment and scoring are independent and order-free.
  * Offline & verifiable: CVE references come from a committed local cache
    (backend/cve_cache.json) built deterministically by
    backend/scripts/build_cve_cache.py. No network calls, no live API.
    CVE ids are real and NVD/Cisco-verifiable; never invented.
  * Caveated & heuristic: enrichment matches a *configuration condition* this
    finding flags to a published advisory's documented precondition. It never
    claims the audited device's own version is confirmed exploitable, and every
    reference carries a `caveat` string that the product UI MUST surface
    verbatim next to the reference.
  * Preregistered: cve_cache.json (the data the validating test checks) must be
...
def load_cve_cache() -> Dict[str, List[dict]]:
    """rule_id -> [CVE entries...] loaded from the committed offline cache."""
...
def enrich_findings(findings: List[FindingDTO]) -> List[FindingDTO]:
    ...
    list attached (CVEReferenceDTO per matching rule) for FAILED findings whose
    rule has cached references -- and only then. It never mutates the input
    finding objects)Skip, never changes severity/weight/status/evidence/
    remediation/explanation, and never alters the audit score or verdicts.
    Enriching the same set of findings always produces a byte-identical
